...
export function SolarScoreTeaser({
  ...
  const getSolarGrade = (score: number) => {
    if (score >= 85)
      return {
        label: 'Excellent',
        badgeClass: 'bg-amber-600 text-white border border-amber-300 shadow-[0_0_18px_rgba(217,119,6,0.35)]',
  ...
  return (
    <div className="animate-in slide-in-from-bottom duration-500 bg-gradient-to-r from-amber-50 via-white to-yellow-50 rounded-2xl border border-amber-100 p-4">
      ...
      <div className="grid gap-3 lg:grid-cols-[1.15fr,0.85fr]">
        <div className="relative overflow-hidden rounded-2xl border border-amber-100 bg-white p-4 shadow-sm">
          <div className="absolute inset-0 z-0 pointer-events-none bg-gradient-to-br from-amber-50 via-transparent to-yellow-50 opacity-45" />
          <div className="relative z-10 flex flex-col gap-4">
            <div className="flex flex-wrap items-start justify-between gap-4">
              ...
              <div className="rounded-2xl border border-amber-100 bg-amber-50/70 p-3 text-right text-xs text-amber-800">
                <p className="text-[10px] font-semibold uppercase tracking-[0.25em] text-amber-600">Top-tier ranking</p>
                <p className="mt-2 text-2xl font-semibold text-amber-900">Top {topPercent}%</p>
                <p className="text-[11px] text-amber-700">in the United States</p>
                <p className="mt-2 text-[11px] text-amber-700/80">Based on sun exposure and roof size in your area.</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div className="relative h-20 w-20">
                <div
                  ...
                  style={{ background: `conic-gradient(#d97706 ${gaugeAngle}, #fffbeb ${gaugeAngle})` }}
                />
                <div className="absolute inset-2 rounded-full bg-white shadow-inner flex items-center justify-center">
                  <span className="text-2xl font-black text-amber-700">{normalizedScore}</span>
                </div>
              </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
          ...
          {showSavings && (
            <div className="rounded-2xl border border-amber-200 bg-amber-50/70 p-4 flex flex-col gap-2">
              <div className="flex items-center gap-2 text-amber-700">
                <DollarSign className="h-4 w-4" />
                <p className="text-[11px] uppercase tracking-wide font-semibold">Projected year-one savings</p>
              </div>
              <p className="text-3xl font-bold text-amber-700">{currencySymbol}{formattedMin} – {currencySymbol}{formattedMax}</p>
              <p className="text-xs text-amber-700">Roughly {currencySymbol}{monthlyMin} – {currencySymbol}{monthlyMax} off your monthly bill once the system is live.</p>
            </div>
          )}
        </div>
      </div>
