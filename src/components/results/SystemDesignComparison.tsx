...
export function SystemDesignComparison({
  ...
  const tierStyles: Record<number, { badge: string; badgeBg: string; accent: string }> = {
    ...
    1: { badge: "Recommended", badgeBg: "bg-amber-100 text-amber-700", accent: "border-amber-400" },
    ...
    <div className="relative">
      ...
      {toast && (
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-50 max-w-md animate-in fade-in slide-in-from-top-4 duration-300">
          <div className="flex items-start gap-3 rounded-xl border border-amber-200 bg-white px-5 py-4 shadow-xl">
            <span className="mt-0.5 flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-amber-100 text-amber-600 text-sm">✓</span>
          ...
          return (
            <button
              ...
              className={`relative rounded-xl border-2 px-5 py-6 text-left transition-all ${
                isSelected
                  ? 'border-amber-500 bg-amber-50/50 shadow-md ring-1 ring-amber-200'
                  : `${tier.accent} hover:shadow-sm hover:border-amber-300`
              }`}
            >
              {/* Tier badge */}
              <div className="flex items-center justify-between mb-3">
                ...
                {isRecommended && !isSelected && (
                  <span className="text-[10px] text-amber-600 font-medium">★ Best value</span>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-x-3 gap-y-2 text-xs mb-4">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-amber-600">{option.roi25Year}%</span>
                </div>
              </div>
              ...
              <div className="rounded-lg bg-amber-50 px-3 py-2 flex items-center justify-between">
                <span className="text-[11px] text-amber-700">Year 1 savings</span>
                <span className="text-sm font-bold text-amber-700">${option.firstYearSavings.toLocaleString()}</span>
              </div>
            </button>
          );
        })}
